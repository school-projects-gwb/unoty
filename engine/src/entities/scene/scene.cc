...
class Scene::Impl {
 public:
  ...
  void RenderObjects(const std::unique_ptr<ui::Renderer> &renderer) {
    ...
    // filter by viewport
    if (viewport_rendering_) game_objects = FilterObjectsInView(game_objects);
 ...
 private:
  ...
  bool CheckIfObjectInView(const std::shared_ptr<GameObject> &object) {
    ...
    if (dynamic_cast<UiObject *>(object.get())) {
      return true;
    }

    if (object_size.x <= 0 || object_size.y <= 0) {
      return true;
    }

    if (camera_->IsObjectInViewport(object_transform, viewport_offset_, camera_->GetPosition(true))) {
      return true;
    }

    if(CheckEmitterInView(object)){
      return true;
    }
